...
async function shareQR() {
    try {
        ...
        const shareData = {
            title: 'QR Code',
            text: `Check out this QR code: ${text.length > 50 ? text.substring(0, 50) + '...' : text}`,
            files: [file]
        };
        ...
        if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
            ...
            await navigator.share({
                title: 'QR Code',
                text: `Check out this QR code content: ${text}`,
                url: window.location.href
            });
        } else {
            // Fallback for browsers without Web Share API
            await fallbackShare(canvas, text);
        }
    } catch (error) {
        ...
        try {
            const canvas = generateDownloadQR();
            await fallbackShare(canvas, getCurrentContent());
        } catch (fallbackError) {
            console.error('Fallback share failed:', fallbackError);
            alert('Sharing not supported on this device. You can download the QR code instead.');
        }
    }
}

// Fallback share method for browsers without Web Share API
async function fallbackShare(canvas, text) {
    try {
        ...
        if (navigator.clipboard && navigator.clipboard.write) {
            ...
            alert('QR code copied to clipboard! You can now paste it in any app.');
        } else {
            // Last resort: trigger download
            const link = document.createElement('a');
            link.download = 'shared-qrcode.png';
            link.href = canvas.toDataURL('image/png');
            link.click();
            alert('QR code downloaded. You can now share the downloaded file.');
        }
    } catch (error) {
...
// Share saved QR code
async function shareSavedQR(id) {
    try {
        ...
        request.onsuccess = async function() {
            ...
            if (qr) {
                ...
                const fileName = qr.fileName || `qrcode_${qr.id}`;
                const file = new File([blob], `${fileName}.png`, { type: 'image/png' });
                
                // Prepare share data
                const shareData = {
                    title: 'QR Code',
                    text: `Check out this QR code: ${qr.text.length > 50 ? qr.text.substring(0, 50) + '...' : qr.text}`,
                    files: [file]
                };
                ...
                if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
                    ...
                    await navigator.share({
                        title: 'QR Code',
                        text: `Check out this QR code content: ${qr.text}`,
                        url: window.location.href
                    });
                } else {
                    ...
                    if (navigator.clipboard && navigator.clipboard.write) {
                        ...
                        link.href = qr.image;
                        link.click();
                        alert('QR code downloaded for sharing.');
                    }
                }
            }
        };
    } catch (error) {
        console.error('Error sharing saved QR code:', error);
        alert('Unable to share QR code. You can download it instead.');
    }
}
function downloadQR() {
    const canvas = generateDownloadQR();
